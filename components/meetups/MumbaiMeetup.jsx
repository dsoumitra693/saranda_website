...
export default function MumbaiMeetup() {
  ...
  return (
    ...
    >
      <h2 className={`text-3xl font-bold text-white mb-4 transform transition-all duration-700 delay-300 ${
        animationPhase >= 1 ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'
      }`}>Mumbai MeetUp</h2>
      <div className={`text-white mb-6 transform transition-all duration-700 delay-500 ${
        animationPhase >= 2 ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
      }`}>
        <p className="text-lg mb-2 text-yellow-200"><strong>Date:</strong> 31 Jan 2025</p>
        <p className="text-base leading-relaxed text-gray-200">
          We had an amazing meetup in Mumbai! Our community came together to network, share ideas, and make meaningful 
          connections. Check out the photos from this wonderful gathering.
        </p>
      </div>
      
      <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 transform transition-all duration-700 delay-700 ${
        ...
        {[
          { bg: 'from-slate-400 to-slate-600', content: (
            <div className="text-center text-white p-2">
              ...
              <div className="text-xs mt-1">MEET UP AT MUMBAI</div>
            </div>
          ), delay: '0ms' },
          { bg: 'from-purple-400 to-purple-600', content: <span className="text-sm">Mumbai Group Photo 1</span>, delay: '100ms' },
          { bg: 'from-teal-400 to-teal-600', content: <span className="text-sm">Mumbai Group Photo 2</span>, delay: '200ms' },
          { bg: 'from-red-400 to-red-600', content: <span className="text-sm">Mumbai Activities</span>, delay: '300ms' }
        ].map((photo, index) => (
          <div 
            key={index}
